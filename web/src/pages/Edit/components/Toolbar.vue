  ...
  <div class="toolbarContainer" :class="{ isDark: isDark }">
    <div class="toolbar" ref="toolbarRef">
      ...
      <div class="toolbarBlock">
        ...
        <el-tooltip
          effect="dark"
          content="AI激进挖空：大量挖空关键词，适合高强度记忆测试"
          placement="bottom"
        >
          <div
            class="toolbarBtn"
            :class="{ disabled: aiClozeLoading }"
            @click="doAggressiveCloze"
          >
            <span class="icon el-icon-lightning" :style="{ color: aiClozeLoading ? '' : '#e74c3c' }"></span>
            <span class="text">{{ aiClozeLoading && aiClozeMode === 'aggressive' ? '挖空中...' : 'AI激进挖空' }}</span>
          </div>
        </el-tooltip>
        <!-- 清除挖空 -->
        <el-tooltip
      ...
      <div class="aiClozeMask" v-if="aiClozeLoading">
        <div class="aiClozeLoadingBox">
          ...
          <span>AI 正在分析思维导图并{{ aiClozeMode === 'aggressive' ? '激进' : '智能' }}挖空...</span>
        </div>
      </div>
    </div>
    <NodeImage></NodeImage>
...
export default {
  ...
  mounted() {
    ...
    // 挖空版本管理初始化
    this.clozeVersions = getClozeVersions()
    ...
    this.$bus.$on('ai_aggressive_cloze_nodes', this.onAggressiveClozeNodes)
  },
  beforeDestroy() {
    ...
    this.$bus.$off('ai_aggressive_cloze_nodes', this.onAggressiveClozeNodes)
  },
  methods: {
    // 切换复习模式
    toggleReviewMode() {
    ...
    async doAggressiveCloze() {
      ...
      this.aiClozeLoading = true
      this.aiClozeMode = 'aggressive'
      try {
        const count = await aggressiveCloze(this.aiConfig)
        this.$notify({
          title: '激进挖空',
          message: `AI 已完成激进挖空，共挖空 ${count} 个节点`,
          type: 'success',
          duration: 3000
        })
        this.refreshClozeContent()
    ...
    // 右键激进挖空指定节点
    async onAggressiveClozeNodes(nodes) {
      if (this.aiClozeLoading) return
      // 检查 AI 配置
      if (!(this.aiConfig.api && this.aiConfig.key && this.aiConfig.model)) {
        this.$bus.$emit('showAiConfigDialog')
      ...
      this.aiClozeMode = 'aggressive'
      try {
        const count = await aggressiveClozeNodes(this.aiConfig, nodes)
        this.$notify({
          title: '激进挖空',
          message: `AI 已完成挖空，共处理 ${count} 个节点`,
          type: 'success',
    ...
    confirmSaveVersion() {
      ...
      const version = saveClozeVersion(name)
      this.clozeVersions = getClozeVersions()
    ...
    removeVersion(versionId) {
      this.$confirm('确认删除此挖空版本？', '提示', {
        ...
        this.clozeVersions = deleteClozeVersion(versionId)
