...
const store = new Vuex.Store({
  state: {
    ...
    localConfig: {
      ...
      lanServerPort: 8080,
      // 复习计划设置
      reviewForgotTime: '23:59', // 忘记复习触发时间
      reviewReminderTime: '20:00', // 定时提醒时间
      reviewWebhookUrl: '', // 飞书webhook地址
      reviewBackupDir: '', // 备份目录路径
      reviewAutoStart: true // 开机自启动
    },
    ...
    aiConfig: {
      // 通用 OpenAI 兼容接口配置：完整接口地址 / API Key / 模型
      api: '',
      key: '',
      model: '',
  ...
  mutations: {
    ...
    // 设置当前显示的侧边栏
    // 设置当前编辑的本地文件路径
    setCurrentFilePath(state, data) {
      state.currentFilePath = data
    },

    // 设置复习模式
    setIsReviewMode(state, data) {
