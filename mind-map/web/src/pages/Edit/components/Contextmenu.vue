<template>
  ...
  >
    <template v-if="type === 'node'">
      ...
      <div class="splitLine" v-if="!isMultiSelect"></div>
      <div
        class="item"
        @click="exec('UP_NODE')"
      ...
      <div class="item danger" @click="exec('REMOVE_CURRENT_NODE')" v-if="!isMultiSelect">
      ...
      <div class="item" @click="exec('PASTE_NODE')" v-if="!isMultiSelect">
        <span class="name">{{ $t('contextmenu.pasteNode') }}</span>
        <span class="desc">Ctrl + V</span>
      </div>
      <div class="splitLine" v-if="!isMultiSelect"></div>
      <div class="item" @click="exec('REMOVE_HYPERLINK')" v-if="hasHyperlink && !isMultiSelect">
        <span class="name">{{ $t('contextmenu.removeHyperlink') }}</span>
      </div>
      <div class="item danger" @click="exec('REMOVE_NOTE')" v-if="hasNote">
      ...
      <div class="item" @click="exec('EXPORT_CUR_NODE_TO_PNG')" v-if="!isMultiSelect">
        <span class="name">{{ $t('contextmenu.exportNodeToPng') }}</span>
      </div>
      <div class="splitLine" v-if="enableAi"></div>
      <div class="item" @click="aiCreate" v-if="enableAi && !isMultiSelect">
        <span class="name">{{ $t('contextmenu.aiCreate') }}</span>
      </div>
      <div class="item" @click="aiSmartCloze" v-if="enableAi">
        <span class="name">AI智能挖空</span>
      </div>
      <div class="item" @click="aiReciteRewrite" v-if="enableAi">
        <span class="name">AI背诵改写</span>
      </div>
      <div class="splitLine"></div>
      <div class="item" @click="showSelectedCloze" v-if="hasClozeNodes">
        <span class="name">显示挖空</span>
      </div>
      <div class="item" @click="hideSelectedCloze" v-if="hasClozeNodes">
        <span class="name">隐藏挖空</span>
      </div>
      <div class="item" @click="addToReviewPlan" v-if="!node.isRoot">
        <span class="name">添加到复习计划</span>
      </div>
      <div class="item" @click="addNodesToAiChat" v-if="enableAi">
        <span class="name">将节点添加到AI对话</span>
      </div>
    </template>
...
import { setNodesClozeHidden, nodeHasCloze } from '@/utils/cloze'
...
export default {
  ...
  computed: {
    ...
    // 当前右键作用的节点列表（框选多个或单个）
    selectedNodes() {
      const activeNodes = this.mindMap ? (this.mindMap.renderer.activeNodeList || []) : []
      return activeNodes.length > 0 ? activeNodes : (this.node ? [this.node] : [])
    },
    // 选中节点中是否有挖空内容
    hasClozeNodes() {
      return this.selectedNodes.some(n => nodeHasCloze(n))
    },
    expandList() {
      return [
    ...
    isMultiSelect() {
      return this.mindMap && this.mindMap.renderer && this.mindMap.renderer.activeNodeList.length > 1
    },
    hasHyperlink() {
      return !!this.node.getData('hyperlink')
    },
    hasNote() {
      return this.selectedNodes.some(n => !!n.getData('note'))
    },
    numberTypeList() {
  ...
  methods: {
      ...
      switch (key) {
        ...
        case 'UNEXPAND_ALL':
          if (this.isMultiSelect) {
            ...
            activeNodes.forEach(n => {
              this.$bus.$emit('execCommand', key, !n.uid, n.uid)
            })
          } else {
        ...
        case 'EXPAND_ALL':
          if (this.isMultiSelect) {
            const activeNodes = this.mindMap.renderer.activeNodeList
            activeNodes.forEach(n => {
              this.$bus.$emit('execCommand', key, n.uid)
            })
          } else {
    ...
    aiSmartCloze() {
      const activeNodes = this.mindMap.renderer.activeNodeList || []
      const nodes = activeNodes.length > 1 ? activeNodes : [this.node]
      this.$bus.$emit('ai_smart_cloze_nodes', nodes)
      this.hide()
    },
    ...
    aiReciteRewrite() {
      const activeNodes = this.mindMap.renderer.activeNodeList || []
      const nodes = activeNodes.length > 1 ? activeNodes : [this.node]
      this.$bus.$emit('ai_recite_rewrite_nodes', nodes)
      this.hide()
    },
    ...
    addToReviewPlan() {
      const activeNodes = this.mindMap.renderer.activeNodeList || []
      const nodes = activeNodes.length > 1 ? activeNodes : [this.node]
      this.$bus.$emit('add_to_review_plan', nodes)
      this.hide()
    },

    // 二开：将选中节点添加到 AI 对话输入框（灰色引用样式）
    // 如果 AI 对话侧边栏未打开，自动打开
    addNodesToAiChat() {
      const nodes = this.selectedNodes
      if (!nodes || nodes.length === 0) return
      ...
      this.$bus.$emit('add_nodes_to_ai_chat', nodes)
      this.hide()
    },

    // 显示选中节点的挖空内容
    showSelectedCloze() {
      setNodesClozeHidden(this.selectedNodes, false)
      this.hide()
    },

    // 隐藏选中节点的挖空内容
    hideSelectedCloze() {
      setNodesClozeHidden(this.selectedNodes, true)
      this.hide()
    }
  }
}
