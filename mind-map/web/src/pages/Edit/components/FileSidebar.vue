...
export default {
  ...
  created() {
    ...
    // 进入复习模式时自动收起目录树
    this.$bus.$on('enter_review_mode', this.collapseOnReview)
  },
  beforeDestroy() {
    if (!this.isElectron) return
    this.$bus.$off('data_change', this.onDataChange)
    this.$bus.$off('lanSyncFileState', this.onLanSyncFileState)
    this.$bus.$off('enter_review_mode', this.collapseOnReview)
  ...
  methods: {
    ...
    async createFileOnDesktop() {
      try {
        ...
        // 重载桌面节点子树并展开（不再清空整棵树），展开后进入内联编辑
        this.reloadDesktopAndEdit(desktop, target)
      } catch (e) {
        console.log(e)
    ...
    // 重载桌面节点子树、展开后进入内联编辑
    // 避免 refreshTree() 清空整棵树导致所有目录被折叠、新文件不可见
    reloadDesktopAndEdit(desktopPath, filePath) {
      const tree = this.$refs.fileTreeRef
      const startEdit = () => {
        this.startInlineEdit(filePath, filePath.split(/[\\/]/).pop())
      }
      if (!tree) {
        startEdit()
        return
      }
      const expandAndEdit = node => {
        node.loaded = false
        node.expand(() => {
          this.$nextTick(startEdit)
        })
      }
      let desktopNode = tree.getNode(desktopPath)
      if (desktopNode) {
        // 桌面节点已存在：只重载桌面子树，保持其他节点展开状态不变
        expandAndEdit(desktopNode)
      } else {
        // 桌面是新增根：需要刷新整棵树后才能找到它
        this.refreshTree()
        const tryFind = attempts => {
          ...
          this.$nextTick(() => {
            desktopNode = tree.getNode(desktopPath)
            if (desktopNode) {
              expandAndEdit(desktopNode)
            } else {
    ...
    // 注意：不能用 path 拼接找父节点（尾分隔符会导致 getNode 匹配失败 → 走 refreshTree 清空整棵树，所有展开状态丢失），
    // 改用当前节点的 parent 引用定位父节点
