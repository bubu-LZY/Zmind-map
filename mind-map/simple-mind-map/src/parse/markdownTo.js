...
// 创建节点数据，支持 [==文本==] 挖空语法（二开功能）
// 包含挖空语法的文本会生成富文本节点，挖空部分包裹 smm-cloze span
const createNodeData = text => {
  if (/\[==[\s\S]+?==\]/.test(text)) {
    const html =
      '<p>' +
...
const handleList = node => {
  ...
  let walk = (arr, newArr) => {
    for (let i = 0; i < arr.length; i++) {
      let cur = arr[i]
      let node = {}
      node.data = createNodeData(getNodeText(cur))
      node.children = []
      newArr.push(node)
...
export const transformMarkdownTo = md => {
  const list = transformMarkdownToList(md)
  return list[0]
}

// 将markdown转换成节点树列表（保留所有顶层节点，用于粘贴解析等场景）
export const transformMarkdownToList = md => {
