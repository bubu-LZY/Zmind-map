...
// 挖空编解码（二开）：<code> ↔ <span class="smm-cloze">
// 编辑器用 quill 内建 code 格式（已加入 formats 白名单）存挖空，
// 选中文字时直接 quill.formatText('code') 立即包裹 <code>，编辑器内显示紫色下划线；
// 保存时 clozeEncode 把 <code> 转为 <span class="smm-cloze"> 落盘；
// 加载时 clozeDecode 还原为 <code> 供 quill 解析（code 在白名单内不会被剥离，避免数据丢失）。
...
// 覆盖 Quill Link blot 的 sanitize：默认白名单只有 http/https/mailto/tel，
// 会把 zmind-file:/zmind-node: 改成 #，导致 <a href="#"> 点击打开空白新窗口，
// 且 onRenderLinkClick 的 closest('a[href^="zmind-file:"]') 匹配不到。
;(() => {
  ...
  if (OriginalLink && !OriginalLink._zmindPatched) {
    class ZmindLink extends OriginalLink {
      static sanitize(url) {
        if (
          typeof url === 'string' &&
          (url.startsWith('zmind-file:') || url.startsWith('zmind-node:'))
        ) {
          return url
        }
...
class RichText {
  ...
  bindEvent() {
    ...
    this.onRenderLinkClick = this.onRenderLinkClick.bind(this)
    ...
    document.addEventListener('click', this.onRenderLinkClick, true)
  ...
  unbindEvent() {
    ...
    document.removeEventListener('click', this.onRenderLinkClick, true)
  ...
  appendCss() {
    this.mindMap.appendCss(
      ...
      /* 二开：@ 文件引用 / # 节点引用链接样式 - 灰色底 */
      .smm-richtext-node-wrap a[href^="zmind-file:"],
      .smm-richtext-node-wrap a[href^="zmind-node:"],
      .ql-editor a[href^="zmind-file:"],
      .ql-editor a[href^="zmind-node:"] {
      ...
      .smm-richtext-node-wrap a[href^="zmind-file:"]:hover,
      .smm-richtext-node-wrap a[href^="zmind-node:"]:hover,
      .ql-editor a[href^="zmind-file:"]:hover,
      .ql-editor a[href^="zmind-node:"]:hover {
  ...
  extendQuill() {
    ...
    // 挖空（二开）：改用 quill 内建 code 格式作为载体（已加入 formats 白名单），
    // 不再注册自定义 ClozeBlot（避免 Quill 2.x 下 blots/inline 路径 extends undefined 的崩溃）。
  }
  ...
  showEditText({ node, rect, isInserting, isFromKeyDown, isFromScale }) {
    ...
    if (!this.textEditNode) {
      ...
      this.textEditNode.addEventListener('click', e => {
        e.stopPropagation()
        // 二开：拦截 zmind-file: / zmind-node: 链接点击
        const link = e.target.closest('a[href^="zmind-file:"], a[href^="zmind-node:"]')
        if (link) {
          e.preventDefault()
          const href = link.getAttribute('href')
          // 获取链接元素的屏幕坐标，用于悬浮窗定位
          const linkRect = link.getBoundingClientRect()
          const pos = { x: linkRect.left, y: linkRect.bottom, width: linkRect.width }
          if (href.startsWith('zmind-file:')) {
            this.mindMap.emit('file_link_click', { filePath: href.replace('zmind-file:', ''), pos })
          } else if (href.startsWith('zmind-node:')) {
    ...
    if (isFromKeyDown && autoEmptyTextWhenKeydownEnterEdit) {
      ...
      // 已经是富文本：把落盘的 smm-cloze span 还原为 quill 内建 code 格式
      const raw = this.cacheEditingText || String(nodeText)
      this.textEditNode.innerHTML = clozeDecode(raw)
    }
  ...
  hideEditText(nodes) {
    ...
    const html = clozeEncode(this.getEditText())
  ...
  // 初始化Quill富文本编辑器
  initQuillEditor() {
    const _self = this // 供 quill keyboard bindings 内部引用 RichText 实例
    this.quill = new Quill(this.textEditNode, {
        ...
        keyboard: {
          bindings: {
            enter: {
              ...
              handler: function () {
                // 回车退出编辑（保存当前内容）
                if (_self && _self.showTextEdit) {
                  _self.hideEditText()
                }
              }
            },
            // 注：挖空的 Ctrl+Enter 已改由 Edit.vue 的 document 捕获监听统一处理，
            // 不再在 quill 内绑定，避免与 keyCommand 双触发。
            shiftEnter: {
              key: 'Enter',
              shiftKey: true,
      ...
      formats: [
        ...
        'link',
        'align'
      ], // 明确指定允许的格式，不包含有序列表，无序列表等。code 用作挖空载体，link 用作文件/节点引用
      theme: 'snow'
    })
    ...
    this.quill.on('text-change', (delta) => {
      ...
      // 二开：检测 "@" 或 "#" 输入，触发文件/节点引用搜索弹窗
      if (delta && delta.ops) {
        for (const op of delta.ops) {
          if (op.insert === '@') {
            this.checkAndShowFileMention('file')
            break
          } else if (op.insert === '#') {
            this.checkAndShowFileMention('node')
            break
          }
        }
      }
  ...
  // 二开：检测 "@" 或 "#" 输入位置并触发引用弹窗
  checkAndShowFileMention(mode) {
    if (!this.quill || !this.textEditNode) return
    setTimeout(() => {
      try {
        ...
        if (!bounds) return
        const editorRect = this.textEditNode.getBoundingClientRect()
        const x = editorRect.left + bounds.left
        const y = editorRect.top + bounds.bottom + 4
        this.mindMap.emit('show_file_mention', { x, y, index: range.index, mode })
      } catch (e) {}
    }, 0)
  }

  // 二开：在当前光标位置插入文件/节点引用链接
  insertFileLink(displayText, linkUrl, insertIndex) {
    if (!this.quill) return
    if (insertIndex !== undefined && insertIndex > 0) {
      this.quill.deleteText(insertIndex - 1, 1)
    }
    const index = insertIndex !== undefined ? insertIndex - 1 : this.quill.getSelection().index
    this.quill.insertText(index, displayText, { link: linkUrl })
    this.quill.insertText(index + displayText.length, ' ', {})
    this.quill.setSelection(index + displayText.length + 1, 0)
  }

  // 二开：渲染态点击文件/节点引用链接 → 触发预览（编辑态由 textEditNode 的 click 监听处理）
  // 同时传递点击坐标，用于悬浮窗定位（跟随节点）
  onRenderLinkClick(e) {
    const link = e.target.closest(
      'a[href^="zmind-file:"], a[href^="zmind-node:"]'
    )
    if (!link) return
    if (this.textEditNode && this.textEditNode.contains(e.target)) return
    e.preventDefault()
    e.stopPropagation()
    const href = link.getAttribute('href')
    // 获取链接元素的屏幕坐标，用于悬浮窗定位
    ...
    if (href.startsWith('zmind-file:')) {
      ...
      const sepIdx = data.lastIndexOf(':')
      if (sepIdx > 0) {
        this.mindMap.emit('node_link_click', {
          filePath: data.substring(0, sepIdx),
          nodeUid: data.substring(sepIdx + 1),
          pos
        })
      }
    }
  ...
  // 二开：Backspace/Delete 整体删除文件/节点引用链接（而非逐字符删除）
  onGlobalKeydown(e) {
    if (!this.quill) return
    ...
    // Backspace 检查光标前一个字符，Delete 检查光标处字符
    const checkIndex = isBackspace ? sel.index - 1 : sel.index
    if (checkIndex < 0) return
    const fmt = this.quill.getFormat(checkIndex, 1)
    if (fmt && fmt.link) {
      const range = isBackspace
        ? this.findLinkRange(sel.index)
        : this.findLinkRangeForward(sel.index)
      if (range) {
        e.preventDefault()
  ...
  // 查找连续 link 文本的范围（用于 Backspace 整体删除，向前查找）
  findLinkRange(index) {
    const quill = this.quill
    const targetLink = quill.getFormat(index - 1, 1).link
    if (!targetLink) return null
    const len = quill.getLength()
    let start = index - 1
    while (start > 0 && quill.getFormat(start - 1, 1).link === targetLink) {
      start--
    }
    let end = index
    while (end < len && quill.getFormat(end, 1).link === targetLink) {
      end++
    }
    return { start, end }
  }

  // 查找连续 link 文本的范围（用于 Delete 整体删除，从光标处向后查找）
  findLinkRangeForward(index) {
    const quill = this.quill
    const targetLink = quill.getFormat(index, 1).link
    if (!targetLink) return null
    const len = quill.getLength()
    let start = index
    while (start > 0 && quill.getFormat(start - 1, 1).link === targetLink) {
