...
function getNodeRect() {
  ...
  // 备注：不加入 textContentWidth，备注图标将定位在节点右边缘，不参与文字居中
  if (this._noteData) {
  ...
  // 备注图标额外宽度（定位在节点右边缘，不参与文字居中计算）
  if (this._noteData) {
    _width += 6 + this._noteData.width
  }
...
function layout() {
  ...
  // 备注：不加入 textContentNested，在 translate 后单独定位到节点右边缘
  // 附件
  if (this._attachmentData) {
  ...
  // 备注图标额外宽度，居中计算时需排除，避免文字偏移
  const noteExtraWidth = this._noteData ? 6 + this._noteData.width : 0
  let translateX = 0
  let translateY = 0
  switch (imgPlacement) {
    case IMG_PLACEMENT.TOP:
      translateX = (width - noteExtraWidth) / 2 - bboxWidth / 2
      translateY =
        paddingY + // 内边距
        imgHeight + // 图片高度
  ...
  // 备注图标定位在节点右边缘（紧贴右边框内侧）
  if (this._noteData) {
    const noteX = width - paddingX - this._noteData.width
    const noteY = translateY + (textContentHeight - this._noteData.height) / 2
    this._noteData.node.x(noteX).y(noteY)
    this.group.add(this._noteData.node)
  }
  this.addHoverNode(width, height)
