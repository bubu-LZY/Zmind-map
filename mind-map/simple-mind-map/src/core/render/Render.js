...
class Render {
  ...
  registerShortcutKeys() {
    ...
    // 选中上一个节点（二开功能：深度优先逆序，跳过祖先节点，对齐幕布行为）
    // 二开：Shift+Tab 切换节点时不移动视图（不调用 moveNodeToCenter），
    // 仅展开父节点并激活目标节点，避免思维导图被强制居中
    this.mindMap.keyCommand.addShortcut('Shift+Tab', () => {
      const node = this.activeNodeList[0]
      if (!node) return
      const target = this.getPrevNodeInDfsOrder(node)
      if (target) {
        const uid = target.getData('uid')
        if (!uid) return
        this.expandToNodeUid(uid, () => {
          const targetNode = this.findNodeByUid(uid)
          if (targetNode) {
            targetNode.active()
          }
        })
    ...
    // 上移节点（对齐幕布：Ctrl+Shift+↑）
    this.mindMap.keyCommand.addShortcut('Control+Shift+Up', () => {
      this.mindMap.execCommand('UP_NODE')
    })
    // 下移节点（对齐幕布：Ctrl+Shift+↓）
    this.mindMap.keyCommand.addShortcut('Control+Shift+Down', () => {
    ...
    // 根节点居中显示（文本编辑中不触发，避免与挖空快捷键 Ctrl+Enter 冲突）
    this.mindMap.keyCommand.addShortcut('Control+Enter', () => {
      const te = this.textEdit
      if (te && te.isShowTextEdit && te.isShowTextEdit()) return
      this.setRootNodeCenter()
    })
    // 二开：文字格式快捷键（非编辑态对选中节点整体应用加粗/下划线/斜体）
    // 编辑态由 Quill 内置 Ctrl+B/U/I 处理（TextEdit save/restore 会禁用 Render 快捷键）
    this.mindMap.keyCommand.addShortcut('Control+b', () => {
      this.toggleNodesStyle('fontWeight', 'bold')
    })
    this.mindMap.keyCommand.addShortcut('Control+u', () => {
      this.toggleNodesStyle('textDecoration', 'underline')
    })
    // 二开：非编辑态 Ctrl+I 整个节点斜体（编辑态由 Quill 处理选区斜体）
    this.mindMap.keyCommand.addShortcut('Control+i', () => {
      this.toggleNodesStyle('fontStyle', 'italic')
    })
  }

  // 二开：切换选中节点的文字样式（加粗/下划线等），对整个节点生效
  // 富文本模式下直接修改 Quill HTML 内容，确保视觉生效
  toggleNodesStyle(prop, value) {
    ...
    const tag = propToTag[prop]

    nodes.forEach(node => {
      if (tag && this.mindMap.richText && node.getData('richText')) {
        // 富文本模式：直接修改 HTML 内容
        let html = node.getData('text') || ''
        ...
        div.innerHTML = html
        const hasFormat = div.querySelector(tag) !== null
        const newHtml = this.toggleFormatInHtml(div, tag, !hasFormat)
        node.setText(newHtml, true)
      } else {
        // 非富文本模式：使用 setStyle
        const current = node.getData(prop)
        node.setStyle(prop, current === value ? '' : value)
      }
    })
  }

  // 二开：在 DOM 中切换格式标签（加粗用 <strong>，下划线用 <u>，斜体用 <em>）
  toggleFormatInHtml(container, tag, enable) {
    if (enable) {
      // 先移除已有的同名标签（避免嵌套）
  ...
  // 获取深度优先遍历顺序中的上一个节点（跳过祖先节点，找到上一分支的最深末级节点）
  getPrevNodeInDfsOrder(node) {
    if (!node || !node.parent) return null
    let cur = node
    while (cur.parent) {
      const siblings = cur.parent.children || []
      const index = siblings.findIndex(item => item.uid === cur.uid)
      if (index > 0) {
        // 有上一个同级节点：进入其子树的最深末级节点
