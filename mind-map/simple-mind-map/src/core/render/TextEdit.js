...
export default class TextEdit {
  ...
  //  注册临时快捷键
  registerTmpShortcut() {
    // 每次进入编辑都重新注册，因为 save()/restore() 会交换 shortcutMap 引用
    ...
    this.mindMap.keyCommand.addShortcut('Shift+Tab', () => {
      ...
      if (node) {
        const target = this.mindMap.renderer.getPrevNodeInDfsOrder(node)
        if (target) {
          // 二开：编辑态下 Shift+Tab 同样不移动视图，仅展开并激活目标节点
    ...
    this.mindMap.keyCommand.addShortcut('Esc', () => {
      this.hideEditTextBox()
    })
  }
