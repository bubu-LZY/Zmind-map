...
const LAN_TOKEN_FILE = path.join(app.getPath('userData'), 'lan-token.json')
const LAN_TOKEN_TTL = 30 * 24 * 60 * 60 * 1000 // 30 天有效期
let currentLanToken = ''

function loadLanToken() {
  try {
    const data = JSON.parse(fs.readFileSync(LAN_TOKEN_FILE, 'utf8'))
    if (data && data.token && data.createdAt && Date.now() - data.createdAt < LAN_TOKEN_TTL) {
      currentLanToken = data.token
      return
    }
  } catch (e) {}
  // 不存在或已过期，生成新 token
  currentLanToken = crypto.randomBytes(12).toString('hex')
  try { fs.writeFileSync(LAN_TOKEN_FILE, JSON.stringify({ token: currentLanToken, createdAt: Date.now() })) } catch (e) {}
}

function regenerateLanToken() {
  currentLanToken = crypto.randomBytes(12).toString('hex')
  try { fs.writeFileSync(LAN_TOKEN_FILE, JSON.stringify({ token: currentLanToken, createdAt: Date.now() })) } catch (e) {}
  return currentLanToken
}

function isLanAuthed(req, urlObj) {
  if (!currentLanToken) return false
  const tok = (urlObj && urlObj.searchParams.get('token')) || ''
  if (tok === currentLanToken) return true
  const auth = req.headers.authorization || ''
  if (auth.replace(/^Bearer\s+/i, '') === currentLanToken) return true
  return false
}
    ...
    function doStart() {
      ...
      lanServer = http.createServer(async (req, res) => {
        try {
          ...
          // token 校验端点（前端输入 token 后调用验证）
          if (urlPath === '/api/auth/verify' && req.method === 'GET') {
            const ok = isLanAuthed(req, urlObj)
            res.writeHead(ok ? 200 : 401, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ ok }))
            return
          }

          // 所有 /api/* 接口必须携带正确 token（静态资源不限制，以便前端加载 token 输入页）
          if (urlPath.startsWith('/api/') && !isLanAuthed(req, urlObj)) {
          ...
          fs.readFile(filePath, (err, data) => {
            ...
            if (urlPath === '/index.html') {
              ...
              const injectScript = '<script>' +
                ...
                'var syncTimer=null;' +
                'var lastSentData="{}";' +
                'window._fromSSE=false;' +
                'var TOKEN_KEY="ZMIND_LAN_TOKEN";' +
                'function getToken(){try{return localStorage.getItem(TOKEN_KEY)||""}catch(e){return ""}}' +
                'function setToken(t){try{_set(TOKEN_KEY,t)}catch(e){}}' +
                'function enc(t){return encodeURIComponent(t||"")}' +
                'function showTokenGate(msg){if(document.getElementById("zmindTokenGate"))return;var d=document.createElement("div");d.id="zmindTokenGate";d.style.cssText="position:fixed;left:0;top:0;right:0;bottom:0;background:#1f2329;z-index:2147483647;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif";var box=document.createElement("div");box.style.cssText="width:340px;background:#2b2f36;padding:28px 24px;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.4)";var title=document.createElement("div");title.textContent="请输入访问令牌";title.style.cssText="color:#fff;font-size:16px;font-weight:600;margin-bottom:6px";var tip=document.createElement("div");tip.textContent=msg||"请向桌面端获取局域网访问令牌";tip.style.cssText="color:#9aa0a6;font-size:12px;margin-bottom:16px";var inp=document.createElement("input");inp.type="text";inp.placeholder="输入令牌";inp.style.cssText="width:100%;box-sizing:border-box;padding:10px 12px;border-radius:8px;border:1px solid #3a3f47;background:#1f2329;color:#fff;font-size:14px;outline:none";var err=document.createElement("div");err.style.cssText="color:#ff6b6b;font-size:12px;margin-top:8px;min-height:16px";var btn=document.createElement("button");btn.textContent="验证并进入";btn.style.cssText="width:100%;margin-top:8px;padding:10px;border:none;border-radius:8px;background:#0984e3;color:#fff;font-size:14px;cursor:pointer";box.appendChild(title);box.appendChild(tip);box.appendChild(inp);box.appendChild(err);box.appendChild(btn);d.appendChild(box);document.body.appendChild(d);function verify(){var t=inp.value.trim();if(!t){err.textContent="请输入令牌";return}var x=new XMLHttpRequest();x.open("GET","/api/auth/verify?token="+enc(t),true);x.onload=function(){try{var r=JSON.parse(x.responseText);if(r&&r.ok){setToken(t);location.reload()}else{err.textContent="令牌无效"}}catch(e){err.textContent="验证失败"}};x.onerror=function(){err.textContent="网络错误"};x.send()}btn.onclick=verify;inp.onkeydown=function(e){if(e.key==="Enter")verify()};inp.focus()}' +
                // 注入桌面端初始数据
                'var __LS__=' + lsJson + ';' +
                'if(__LS__){for(var k in __LS__){try{_set(k,__LS__[k])}catch(e){}}}' +
                // 网页端→桌面端：拦截 setItem，防抖发送（带 token；SSE 更新数据不回传防止循环）
                'localStorage.setItem=function(key,val){_set(key,val);if(window._fromSSE)return;if(LS_KEYS.indexOf(key)>=0){if(syncTimer)clearTimeout(syncTimer);syncTimer=setTimeout(function(){var d={};LS_KEYS.forEach(function(k){var v=localStorage.getItem(k);if(v!==null)d[k]=v});lastSentData=JSON.stringify(d);try{var x=new XMLHttpRequest();x.open("POST","/api/sync?token="+enc(getToken()),true);x.setRequestHeader("Content-Type","application/json");x.send(lastSentData)}catch(e){}},300)}};' +
                // 桌面端→网页端：SSE 实时推送（带 token）
                'var es=new EventSource("/api/sse?token="+enc(getToken()));' +
                'es.onmessage=function(event){var t=event.data;if(t===lastSentData||t==="{}")return;lastSentData=t;try{var d=JSON.parse(t);var changed=false;var dataChanged=false;var clozeChanged=false;var collapseChanged=false;var fileChanged=false;window._fromSSE=true;for(var k in d){var c=localStorage.getItem(k);if(c!==d[k]){_set(k,d[k]);changed=true;if(k==="SIMPLE_MIND_MAP_DATA")dataChanged=true;if(k==="SIMPLE_MIND_MAP_CLOZE_STATE")clozeChanged=true;if(k==="SIMPLE_MIND_MAP_COLLAPSE_STATE")collapseChanged=true;if(k==="ZMIND_RECENT_FILES"||k==="ZMIND_FOLDER_ROOTS")fileChanged=true}}if(changed){try{if(window.$bus&&dataChanged&&d.SIMPLE_MIND_MAP_DATA){window.$bus.$emit("lanSyncUpdate",JSON.parse(d.SIMPLE_MIND_MAP_DATA))}if(window.$bus&&clozeChanged){window.$bus.$emit("lanClozeStateChanged")}if(window.$bus&&collapseChanged){window.$bus.$emit("lanCollapseStateChanged")}if(window.$bus&&fileChanged){window.$bus.$emit("lanSyncFileState")}}catch(e){}}setTimeout(function(){window._fromSSE=false},500)}catch(e){window._fromSSE=false}};' +
                'es.onerror=function(){window._fromSSE=false;try{es.close()}catch(e){};if(getToken()){setTimeout(function(){showTokenGate("连接失败，令牌可能已失效，请重新输入")},300)}};' +
                // 未输入 token 时显示令牌输入页
                'if(!getToken()){function _waitBody(){if(document.body){showTokenGate("请向桌面端获取局域网访问令牌")}else{setTimeout(_waitBody,30)}}_waitBody()}' +
                '})();' +
                '</script>'
              html = html.replace(
...
function registerLanIpc() {
  ...
  // 获取局域网访问令牌（供设置界面展示）
  ipcMain.handle('zmind:getLanToken', async () => {
    if (!currentLanToken) loadLanToken()
    return currentLanToken
  })
  // 重新生成局域网访问令牌（旧令牌立即失效）
  ipcMain.handle('zmind:regenerateLanToken', async () => {
    return regenerateLanToken()
  })
...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // 二开：禁用 web 安全策略，允许渲染进程直接 fetch 外部 API（AI 接口等）
      // 避免 CORS 错误（如 tokenhub.tencentmaas.com 等不支持跨域的 API）
      webSecurity: false
    }
  })
  mainWindow = win
